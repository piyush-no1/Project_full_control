...
const ControlDashboard: React.FC = () => {
  ...
  const backendVideoRef = useRef<HTMLImageElement>(null);
  const previewVideoRef = useRef<HTMLVideoElement>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const reconnectTimerRef = useRef<number | null>(null);
  const streamRef = useRef<MediaStream | null>(null);
  const activeModeRef = useRef<'gesture' | 'stylus' | 'none'>('none');
  ...
  const attachPreviewStream = (mediaStream: MediaStream) => {
    if (!previewVideoRef.current) {
      return;
    }
    previewVideoRef.current.srcObject = mediaStream;
    previewVideoRef.current.play().catch(() => {
      // Ignore play interruption if browser blocks autoplay.
    });
  };
  ...
  const startPreview = async () => {
    ...
    try {
      ...
      console.error('Failed to start local camera preview:', error);
      setPreviewReady(false);
      setCameraError('Camera permission is required to show preview.');
    }
  };
  ...
  const connectVideoStream = () => {
    ...
    const ws = new WebSocket(apiService.getVideoWebSocketUrl());
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'frame' && data.frame) {
          ...
          if (backendVideoRef.current) {
            backendVideoRef.current.src = data.frame;
          }
    ...
    ws.onclose = () => {
      setBackendFrameReady(false);
      wsRef.current = null;
      if (activeModeRef.current !== 'none' && reconnectTimerRef.current === null) {
    ...
    ws.onerror = () => {
      // `onclose` handles the actual recovery flow.
    };
        ...
        <div className="relative z-10 w-full max-w-6xl px-10 flex flex-col items-center animate-in fade-in zoom-in duration-700">
          ...
          <div className="w-full relative group mb-12">
            <div className={`relative aspect-video rounded-[3rem] overflow-hidden border-2 transition-all duration-700 ${activeMode !== 'none' ? 'border-primary shadow-[0_0_80px_rgba(13,89,242,0.3)]' : 'border-white/10 bg-white/5 shadow-2xl'}`}>
              ...
              {/* Local camera preview for idle mode */}
              <video
                ref={previewVideoRef}
                autoPlay
                playsInline
                muted
                className={`absolute inset-0 w-full h-full object-contain scale-x-[-1] transition-opacity duration-500 ${(activeMode === 'none' || !backendFrameReady) ? 'opacity-100' : 'opacity-0'}`}
              />

              {/* Video from backend WebSocket for gesture/air-stylus modes */}
              <img 
                ref={backendVideoRef}
                alt="Video Feed"
                className={`absolute inset-0 w-full h-full object-contain transition-opacity duration-700 ${(activeMode !== 'none' && backendFrameReady) ? 'opacity-100' : 'opacity-0'}`}
              />
              ...
              {activeMode === 'none' && !previewReady && (
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  {cameraError && (
                    <p className="mt-3 text-xs text-rose-400 uppercase tracking-widest">{cameraError}</p>
                  )}
                </div>
              )}

              {activeMode !== 'none' && !backendFrameReady && (
