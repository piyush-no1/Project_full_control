...
if not HAND_LANDMARKER_MODEL_PATH.exists():
    ...
    print(f"[Air Stylus Stream] Expected path: {HAND_LANDMARKER_MODEL_PATH}")
    sys.exit(1)
    ...
    for index in candidates:
        for backend in backends:
            ...
            if capture.isOpened():
                ...
                print(f"[Air Stylus Stream] Camera opened on index {index} using {backend_name}")
                return capture
            capture.release()
    return None
...
def run_air_stylus_streaming():
    ...
    if capture is None or not capture.isOpened():
        print("[Air Stylus Stream] ERROR: Could not open camera. Check if another app is using it.")
        return
    ...
    try:
        while capture.isOpened():
            if _should_stop():
                print("[Air Stylus Stream] Stop signal received.")
                break
