...
def _resolve_hand_landmarker_model_path(default_path: str = None) -> str:
    ...
    return str(resolve_hand_landmarker_path())
...
class HandLandmarkExtractor:
    ...
    def __init__(
        self,
        model_path: str = DEFAULT_HAND_LANDMARKER_MODEL_PATH,
        num_hands: int = 1,
...
def collect_gesture_samples(
    ...
    hand_landmarker_model_path: str = DEFAULT_HAND_LANDMARKER_MODEL_PATH,
    num_videos: int = 4,
    ...
    cap = None
    try:
        model_path = hand_landmarker_model_path or DEFAULT_HAND_LANDMARKER_MODEL_PATH
                ...
                if key == ord("s"):
                    for countdown in range(3, 0, -1):
                        ...
                        while time.time() - countdown_start < 1.0:
                            ret, frame = cap.read()
                            if not ret:
                                print("[DataCollector] Frame grab failed during countdown; aborting.")
                                abort = True
                                break
                            frame = cv2.flip(frame, 1)
    ...
    print(f"[DataCollector]   Gesture: '{gesture_name}'")
    print(f"[DataCollector]   Total samples saved: {len(collected_rows)}")
    print(f"[DataCollector]   Saved to: {gesture_csv}")
