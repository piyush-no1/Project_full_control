...
class SLMCodeAgent:
    ...
    def __init__(
        ...
        if not self.api_key:
            self.api_key = os.environ.get("GROQ_API_KEY", "enter api key")
    ...
    def generate_python_script(
        ...
        platform_hint = (
            f"for {target_platform}" if target_platform else "for the target platform"
        )

        prompt = f"""Write a standalone Python 3 script {platform_hint} that does:
"{action_description}"
{gesture_info}
Output ONLY Python code. Use platform-appropriate APIs.
